...
def search_text(query: str, normalized: bool = True,
                surah_hint: int = None, start_after: tuple = None) -> list:
    """
    ...
    Args:
        ...
        surah_hint (int, optional): Surah number to search first; expands ±1, ±3, then full Quran
        start_after (tuple, optional): (surah, ayah) to start search after; fallback to full Quran

    Returns:
        List[QuranVerse]: List of matching verses
    """
    return get_quran_database().search_text(query, normalized, surah_hint=surah_hint, start_after=start_after)


def fuzzy_search(query: str, threshold: float = 0.7, normalized: bool = True,
                max_results: int = None, surah_hint: int = None,
                start_after: tuple = None) -> list:
    """
    ...
    return get_quran_database().fuzzy_search(query, threshold, normalized, max_results,
                                             surah_hint=surah_hint, start_after=start_after)
...
def search_sliding_window(query: str, threshold: float = 80.0, normalized: bool = True,
    ...
    Returns:
        List[MultiAyahMatch]: List of matches sorted by similarity score
...
def smart_search(query: str, threshold: float = 0.7, sliding_threshold: float = 80.0,
                ...
                surah_hint: int = None, start_after: tuple = None) -> dict:
    """
    ...
    if not query or not query.strip():
        return {'method': 'none', 'results': [], 'count': 0}
    ...
    if results:
        ...
        return {'method': 'exact', 'results': limited_results, 'count': len(limited_results)}

    fuzzy_results = fuzzy_search(query, threshold=threshold, normalized=normalized,
                                 max_results=max_results, surah_hint=surah_hint, start_after=start_after)
    if fuzzy_results:
        return {'method': 'fuzzy', 'results': fuzzy_results, 'count': len(fuzzy_results)}

    sliding_results = search_sliding_window(query, threshold=sliding_threshold,
                                            normalized=normalized, max_results=max_results,
                                            surah_hint=surah_hint, start_after=start_after)
    if sliding_results:
        return {'method': 'sliding_window', 'results': sliding_results, 'count': len(sliding_results)}
